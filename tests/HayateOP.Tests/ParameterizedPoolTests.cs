...
public class ParameterizedPoolTests
{
    ...
    public void Capacity_ShouldBePerKeyRatherThanShared()
    {
        ...
        // Two keys, one object each: a shared pool of one could not serve both, and each sub-pool reports
        // the full per-key size rather than a slice of it.
        var a = pools.GetObject("a");
        var b = pools.GetObject("b");
    ...
    public void PerKeySize_ShouldSetTheOptionsItPromises()
    {
        ...
        // Not the default four: sharding divides capacity across shards, and a size of two split four ways
        // would leave shards that can hold nothing at all.
        ...
        using var wide = new ParameterizedHayatePool<string, TestObject>(_ => new TestObject(), 8, name: "wide");
        ...
        using var single = new ParameterizedHayatePool<string, TestObject>(_ => new TestObject(), 1, name: "single");
    ...
    public void Constructor_ShouldRejectArgumentsItCannotUse()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new ParameterizedHayatePool<string, TestObject>(_ => new TestObject(), 0));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            new ParameterizedHayatePool<string, TestObject>(_ => new TestObject(), -1));

        Assert.Throws<ArgumentNullException>(() => CreateKeyedPool().GetObject(null!));
    }

    [Fact(Timeout = 30_000)]
    public void FactoryReturningNull_ShouldFailAtTheCreationBoundary()
    {
        using var pools = new ParameterizedHayatePool<string, TestObject>(_ => null!, 1);
