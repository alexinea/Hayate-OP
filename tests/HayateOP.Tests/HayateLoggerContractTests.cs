...
public class HayateLoggerContractTests
{
    ...
    /// <c>: HayateLoggerBase</c> and the four methods 2.x had gain <c>override</c>. Nothing new has to be
    /// implemented — that is the point of the class, and it is what the reflection guard below pins down.
    /// </summary>
    ...
    public void HidingAnInheritedMember_ShouldNotReachTheInterfaceMap()
    {
        ...
        // The interface call lands on the class that *declared* the implementation, and the hiding method
        // is reachable only through the derived type. This is the mechanism behind the next test: with a
        // virtual body on HayateLoggerBase, the base body would sit in the interface map and a migration
        // that forgot `override` would record nothing while still compiling.
    ...
    public void BaseClass_MustKeepTheTwoXMembersAbstract_AndTheThreeZeroAdditionsConcrete()
    {
        ...
        // ones. A fifth entry would mean 3.0 added a member a migration has to answer for; a missing one
        // would mean a virtual body is back, and with it the silent-drop shape measured above.
