...
/// given a policy — the fluent builder and the container registrations — no longer require <c>new()</c> at
/// compile time, which is what a connection-like type needs. The requirement did not disappear: it moved to
/// the moment the default policy is actually resolved, where it is reported with an actionable message
/// instead of a compile error. Verified: a no-constructor type is pooled through <c>WithPolicy</c> and
/// through the unbounded pool's factory overload, the default policy is still the one that resets and
/// validates (a delegate-based policy would do neither), and the entry points that cannot take a policy
/// keep saying so.
/// </summary>
public class BuilderConstraintTests
{
    ...
    /// <summary>
    /// Carries the two default-policy behaviours a delegate-based policy would not reproduce: the release
    /// hook resets an <see cref="IHayateResettable"/>, and validity follows <see cref="IHayateValidatable"/>.
    /// </summary>
