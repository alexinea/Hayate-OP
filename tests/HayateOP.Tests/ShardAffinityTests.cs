...
/// <summary>
/// Fine-grained affinity strategy -- ShardAffinityMode (None/Thread/Custom).
/// Acceptance: in Thread mode the same thread stably hits the same shard; None keeps the current semantics unchanged;
/// Custom delegate takes effect with safe exception/out-of-range fallback; default None has zero extra hot-path overhead (start index always 0).
/// </summary>
public class ShardAffinityTests
{
    ...
    public void NoneMode_ShouldKeepSequentialSemantics()
    {
        ...
        // None mode: scan sequentially from shard 0 -- the first borrowed object must come from shard 0
    ...
    public void ThreadMode_SameThreadShouldStablyHitAffinityShard()
    {
        ...
        // Repeated borrow/return on the same thread: the first borrow of each round (when the start shard has stock) must hit the same affinity shard.
    ...
    {
        // Verify the start-shard pure function: when the pool is full (every shard has stock), under Thread mode
    ...
    public void CustomMode_OutOfRangeAndThrowingSelectors_ShouldFallBackSafely()
    {
        ...
        // Delegate throws -> falls back to sequential scan, borrow is not interrupted
    ...
    public void CustomMode_WithoutSelector_ShouldFallBackToNoneOnBuild()
    {
        ...
        // An unknown enum value also falls back to None
        ...
        // The Builder should fail fast when an unknown enum value is passed
    ...
    public async Task ThreadMode_ConcurrentThreads_ShouldBorrowWithoutConflict()
    {
        ...
        // Concurrent borrow/return across threads: affinity only changes the scan start, not concurrency correctness
    ...
    /// <summary>The borrowed object's ShardIndex is recorded on the borrow path and read via registry reflection.</summary>
