...
namespace HayateOP.Tests.HealthCheck;

/// <summary>
/// Health check granularity (B4): what the check reports, and on what evidence.
/// Acceptance: an open circuit breaker is no longer reported as healthy; an injected object-level probe can
/// fail the check on the object's own answer; the payload carries the counters it used to omit; and a pool
/// whose slots are merely busy is degraded, not unhealthy — and is not probed at all, because probing it
/// would mean waiting for a slot, which is precisely the state the degraded answer is meant to report.
/// </summary>
public class HayateOpHealthCheckTests
{
    ...
    public async Task CircuitBreakerOpen_ShouldBeReportedAsUnhealthy()
    {
        ...
        Assert.Contains("circuit breaker", after.Description, StringComparison.OrdinalIgnoreCase);
