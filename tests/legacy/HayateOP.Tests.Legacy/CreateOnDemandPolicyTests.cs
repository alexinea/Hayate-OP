    ...
    public class CreateOnDemandPolicyTests
    {
            ...
            using (var barrier = new Barrier(requests))
            {
                var tasks = Enumerable.Range(0, requests)
                    .Select(i => Task.Run(() =>
                    {
                        ...
                        // Timed from the barrier release, not from the task start: on a 4-core CI runner
                        // the thread pool injects the four participants slowly, and that delay sits
                        // outside Acquire, where it says nothing about the reject policy.
                        var sw = Stopwatch.StartNew();
        ...
        public void CreateOnDemand_BelowCapacityMisses_ShouldEachCreateWithoutWaiting()
        {
            ...
            using (var pool = ConcurrentBurstPoolBuilder("on-demand-concurrent").Build())
            {
                ...
                // The contract is per request: a wait-then-create degradation spends the whole 2s acquire
                // timeout inside Acquire, so the slowest acquire is what has to stay in the milliseconds.
                // The old burst wall-clock bound also counted the thread pool's scheduling of the four
                // barrier participants (1984ms on the net48 leg) and failed a run whose acquires were all
                // fast -- a burst total below the 2s timeout already proved no request waited it out.
