    ...
    public class MetricsGatingTests
    {
        ...
        public void MetricsOff_ShouldStillCountBorrowsButNotTheGatedCounters()
        {
                ...
                .Build())
            {
                ...
                // G-1: the operational members follow the same switch. ReuseEfficiency would read a perfect
                // 1.0 here if it were derived from the counters alone, which is why the gate is part of the
                // reading rather than something the caller is expected to remember.
        ...
        public void DiagnosticsOff_ShouldZeroEveryCounterIncludingTotalAcquired()
        {
                ...
                .Build())
            {
                ...
                // G-1: the master switch closes the operational members with everything else, and here the
                // gate really is doing work -- three objects were borrowed and returned through this read.
                Assert.False(stats.MetricsEnabled);
                Assert.Equal(0, stats.PeakActiveObjects);
                Assert.Equal(default(DateTimeOffset), stats.StartedAt);
                Assert.Null(stats.LastActivityTime);
        ...
        private static long MeasureWarmedBorrowAllocations(bool diagnostics, IHayateLogger? logger = null)
        {
            ...
            // One warm-up pass is not enough. The first pass finds an empty pool and runs the cold-boot
            // create path, so "take an existing object off the free list" is still cold when the measured
            // loop starts - and the first run of that path allocates once (64 bytes; reproduced on both
            // net6.0 and net8.0), which lands inside the first measured window. Rounds 1..199 read 0, so it
            // is a cold-start artefact rather than a per-borrow allocation. Measured on this machine: one
            ...
            using (var pool = builder.Build())
            {
                ...
                // Warm up to a genuine steady state: nothing is created or destroyed from here on, and the
                // free-list path has already run outside the measured windows.
