    ...
    public class ObjectMetadataTests
    {
        ...
        public void LastGetThreadId_ShouldIdentifyTheBorrowingThread()
        {
            using (var pool = BaseBuilder().Build())
            {
                ...
                // Borrowed from another thread, the metadata names that thread instead. A dedicated thread
                // rather than a pool task: waiting on a task can inline it onto the waiting thread, which
                // would make both ids the same and the assertion meaningless.
                var observed = 0;
                var expected = 0;
