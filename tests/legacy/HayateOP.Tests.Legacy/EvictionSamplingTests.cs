...
{
    /// <summary>
    /// Eviction sampling changed from "call GetAll().ToArray() every time" to reusing the buffered SnapshotHead(buffer, limit),
    /// avoiding a full array distribution over the entire idle linked list on every pass (long-tail allocation). This case verifies that, under a bounded sampling window,
