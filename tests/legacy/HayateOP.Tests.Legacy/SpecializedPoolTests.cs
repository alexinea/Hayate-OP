    ...
    public class SpecializedPoolTests
    {
        ...
        public void StreamPool_DisposeShouldReturnExactlyOnce()
        {
            using (var pool = new MemoryStreamPool(4))
            {
                ...
                // A second dispose of the same borrow is a no-op: double-returning would park the stream
                // twice and hand the same stream to two borrowers later.
                stream.Dispose();
        ...
        public void StreamPool_OversizedStreamShouldBeDestroyedOnReturn()
        {
            using (var pool = new MemoryStreamPool(4))
            {
                ...
                // The oversized stream was destroyed on return, not parked; the next two borrows are
                // served by fresh streams, never by the oversized one.
                var a = pool.GetObject();
                var b = pool.GetObject();
        ...
        public void StringBuilderPool_DisposeShouldReturnExactlyOnce()
        {
            using (var pool = new StringBuilderPool(4))
            {
                ...
                // A second dispose of the same borrow is a no-op: double-returning would park the builder
                // twice and hand the same builder to two borrowers later.
                sb.Dispose();
        ...
        public void StringBuilderPool_OversizedBuilderShouldBeDestroyedOnReturn()
        {
            using (var pool = new StringBuilderPool(4))
            {
                ...
                // The oversized builder was destroyed on return, not parked; the next two borrows are
                // served by fresh builders, never by the oversized one.
                var a = pool.GetObject();
                var b = pool.GetObject();
                Assert.NotSame(oversized, a);
                Assert.NotSame(oversized, b);
        ...
        {
            using (var secondDone = new System.Threading.ManualResetEventSlim(false))
            {
                ...
                {
                    // The shared builder is exclusively held: this acquires only after the first scope
                    // released.
                    using (var scope = SharedStringBuilder.Acquire())
                    {
                        secondDone.Set();
                ...
                Assert.True(second.Wait(TimeSpan.FromSeconds(20)),
                    "The second acquirer should get in after the first released.");
                second.Dispose();
            }
        }
        ...
        public void SharedStringBuilder_DisposedScopeShouldThrow()
        {
            ...
            Assert.Throws<ObjectDisposedException>(() => { var b = scope.StringBuilder; });
